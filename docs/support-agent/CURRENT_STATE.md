...
V2.3 Lightweight Semantic Evidence Selector starts from the V2.2 checkpoint with a bounded provider-independent one-shot classifier contract and offline evaluator. OpenAI Codex OAuth/configuration became available and the immutable FIRST real run occurred, but its empty extracted text was not a valid capability measurement: subsequent I/O and envelope diagnostics showed Pi `stopReason: "error"` envelopes with no content. The adapter now classifies those envelopes as provider failures rather than invalid model output. The selector is not integrated into `SupportAgentRuntime`; the V2.3 Gate is not passed, and **SEMANTIC CAPABILITY NOT YET VALIDLY EVALUATED**. The current authority is [PROVIDER_ERROR_CLASSIFICATION.md](../v2.3-semantic-selector/PROVIDER_ERROR_CLASSIFICATION.md).
